          ...
          <GanttContentHeader
            ...
            renderHeaderItem={(item: number) => (
              <div className="flex items-center justify-center gap-1">
                <p>{format(addDays(new Date(year.year, index, 1), item), "d", { locale: fr })}</p>
                <p className="text-muted-foreground">
                  {format(addDays(new Date(year.year, index, 1), item), "EEEEE", { locale: fr })}
                </p>
              </div>
            )}
            title={format(new Date(year.year, index, 1), "MMMM yyyy", { locale: fr })}
          />
          <GanttColumns
            columns={month.days}
  ...
  return gantt.timelineData.map((year) => (
    <div className="relative flex flex-col" key={year.year}>
      <GanttContentHeader
        ...
        renderHeaderItem={(item: number) => <p>{format(new Date(year.year, item, 1), "MMM", { locale: fr })}</p>}
        title={`${year.year}`}
      />
    ...
    year.quarters.map((quarter, quarterIndex) => (
      <div className="relative flex flex-col" key={`${year.year}-${quarterIndex}`}>
        <GanttContentHeader
          ...
          renderHeaderItem={(item: number) => (
            <p>{format(new Date(year.year, quarterIndex * 3 + item, 1), "MMM", { locale: fr })}</p>
          )}
          title={`T${quarterIndex + 1} ${year.year}`}
        />
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({ feature, onSelectItem, className }) => {
  ...
  const duration = tempEndAt
    ? formatDistance(feature.startAt, tempEndAt, { locale: fr })
    : `${formatDistance(feature.startAt, new Date(), { locale: fr })} jusqu'à présent`
...
export const GanttSidebarHeader: FC = () => (
  ...
  >
    ...
    <p className="flex-1 truncate text-left">Projets</p>
    ...
    >
      ...
      >
        ...
        <div className="whitespace-nowrap rounded-full border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg">
          {formatDate(date, "dd MMM yyyy", { locale: fr })}
        </div>
      </div>
    </div>
  )
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({ onMove, children, className, ...feature }) => {
  ...
  const itemRef = useRef<HTMLDivElement>(null) // Ref to the feature item for positioning
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        <div
          className="absolute z-50 p-2 bg-card text-card-foreground rounded-md shadow-lg border border-border"
          style={{
            top: "calc(100% + 8px)", // Position below the feature item
            left: "50%", // Center horizontally
            transform: "translateX(-50%)",
            minWidth: "200px",
            pointerEvents: "none", // Ensure it doesn't block clicks on the feature item
          }}
        >
          <p className="font-semibold text-sm">{feature.name}</p>
          <p className="text-xs text-muted-foreground">
            Début: {format(feature.startAt, "dd MMM yyyy", { locale: fr })}
          </p>
          <p className="text-xs text-muted-foreground">Fin: {format(feature.endAt, "dd MMM yyyy", { locale: fr })}</p>
          <p className="text-xs" style={{ color: feature.status.color }}>
            Statut: {feature.status.name}
          </p>
        </div>
      )}
    </div>
